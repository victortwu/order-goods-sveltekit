...
<div class={containerStyle}>
	<div>
		<div class="flex justify-between items-start">
			<h4 class="mb-3 text-xl font-semibold tracking-tight">
				{item.name}
				{isOnList ? `(${quantityType.number} ${quantityType.caseOrUnit})` : ''}
			</h4>
			{#if isEdit && !isOnList}
				<div class="flex">
					<GreenCheckmark />
		...
		{#if expanded}
			...
			<button
				...
				class="float-right bg-transparent hover:text-white text-slate-400 font-bold py-2 px-4"
				><UpChevron /></button
			>
		{:else if isEdit}
			<!-- edit button -->
		...
		{:else}
			...
			<button
				on:click={toggleAccordion}
				class="float-right text-3xl bg-transparent hover:text-white text-slate-400 font-bold py-2 px-4"
				><DownChevron /></button
			>
		{/if}
	</div>
	...
	{#if expanded}
		<div
			...
			class="flex md:p-0 w-full transform transition duration-300 ease-in-out"
		>
			<ItemForm {item} {quantityType} {isEdit} bind:expanded />
		</div>
	{/if}
</div>
